  ...
  async getAllRecords() {
    try {
      ...
      if (typeof firebase.functions === "function") {
        try {
          // Use Cloud Function for secure data access
          const getRecords = firebase.functions().httpsCallable("getRecords");
          const result = await getRecords();
          return result.data.records || [];
        } catch (funcError) {
          console.warn(
            "Cloud Functions not available, falling back to direct access:",
            funcError
          );
        }
      }

      // Fallback: Direct database access (less secure but works without Cloud Functions)
